...
void setup_paging() {
  ...
  // for (u32 frame_address = _4KB; frame_address >= _4KB; frame_address +=
  // _4KB) {
  ...
  // // color_printf(purple1, "...\n  call __setup_paging...\n");
  u32       start_addr  = PtrPDEs;
  const u32 default_PDE = 0b11 | (1 << 7);
  for (u32 address = 0; start_addr < PtrPDEs + _4B * 1024; address += _4MB) {
    const u32 PDE     = default_PDE | address;
    *(u32*)start_addr = PDE;
    start_addr += _4B;
  }
